...
const Skills = () => {
  ...
  const animatedStateRef = useRef(null)
  const trailRef = useRef([])
  const currentImageIndexRef = useRef(0)
  const mousePosRef = useRef({ x: 0, y: 0 })
  const lastMousePosRef = useRef({ x: 0, y: 0 })
  const interpolateMousePosRef = useRef({ x: 0, y: 0 })
  ...
  useEffect(() => {
    const config = {
      ...
      staggerOut: 25,
      slideDuration: 1000,
      slideEasing: "cubic-bezier(0.25, 0.46, 0.45, 0.04)",
      easing: "cubic-bezier(0.67, 0, 0.13, 1)",
    }

    const images = iconsList.map((icon) => icon.image)
    ...
    const createTrailImage = () => {
      ...
      const rect = trailContainer.getBoundingClientRect()
      const startX = interpolateMousePosRef.current.x - rect.left - 87.5
      const startY = interpolateMousePosRef.current.y - rect.top - 87.5
    ...
    const render = () => {
      ...
      interpolateMousePosRef.current.x = MathUtils.lerp(
        interpolateMousePosRef.current.x || mousePosRef.current.x,
        mousePosRef.current.x,
        0.1
      );
      interpolateMousePosRef.current.y = MathUtils.lerp(
        interpolateMousePosRef.current.y || mousePosRef.current.y,
        mousePosRef.current.y,
        0.1
      );
      ...
      removeOldImages()
      animatedStateRef.current = requestAnimationFrame(render)
    }
    ...
    const stopAnimation = () => {
      if (animatedStateRef.current) {
        cancelAnimationFrame(animatedStateRef.current)
        animatedStateRef.current = null
      }
  ...
  return (
    <div ref={trailContainerRef} className='trail-container'></div>
  )
}
